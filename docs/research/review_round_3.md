...
materializer 对 merge / create / provisional / defer 四类决策做确定性物化；
...
> 实体解析需要依赖当前图结构，而当前图结构又由实体解析结果不断更新。在并发 LLM mention 流下，这种解析—结构循环依赖会导致不可重放、错误合并、重复实体和图结构污染。
...
4. **热点实体长尾阻塞**：大量 mention 指向同一高频实体时，锁竞争严重。
5. **新实体漏识别或乱创建**：系统要么过度保守、拒绝新增，要么把低置信 mention 直接变成永久实体。
...
## 8. 物化层实现备注：稳定 ID 与集合式写入

物化层需要保证重试安全，这是标准工程实践，不作为本文核心贡献，但对系统正确性不可或缺。
...
CRDT-like 思路仅适用于 alias set、evidence set 等单调字段的集合式合并。实体合并是语义决策，不是集合 union，不应声称”CRDT 解决实体合并”。
...
2. provisional layer 在不显著增加 wrong merge 的情况下提升新实体召回；
3. defer queue 降低错误合并，但不能无限增大 unresolved rate；
4. mention clustering 减少新实体碎片化。
...
提出并形式化 LLM-extracted KG ingestion 中的 Resolution-Structure Circular Dependency，指出实体解析依赖动态图结构，而图结构又由并发解析决策持续更新，从而导致不可重放、错误合并传播和图结构污染。
...
设计 merge_existing、create_canonical、create_provisional、defer 四分决策机制，以及确定性图物化协议，在保证 canonical KG 稳定性的同时支持新实体受控增长。

### Contribution 4：Benchmark and evaluation

构建面向 LLM-KG ingestion 的评测框架，在 synonym-heavy、homonym-heavy、skew、retry/failure 等场景下联合评估实体解析质量、图稳定性和系统吞吐。
...
| 稳定 ID 与集合式写入 | 工程常识 | 标准实践，不作为核心贡献 |
...
```text
We design an ingestion-layer protocol for deterministic materialization of LLM-extracted mentions into a canonical KG.
```
...
> 本项目不应被包装成普通的 LLM 知识图谱构建系统，而应聚焦于 LLM mention 流并发进入 canonical KG 时的解析—结构循环依赖问题。通过 epoch snapshot resolution、mention clustering、merge/create/provisional/defer 四分决策和确定性物化，可以在支持知识图谱持续增长的同时，降低错误合并、重复实体和不可重放风险。
...
> 本项目研究 LLM 抽取 mention 流在并发物化为 canonical knowledge graph 时的 Resolution-Structure Circular Dependency：实体解析依赖当前图结构，而图结构又由解析决策不断更新，导致不可重放、错误合并和结构污染。我们提出一种不确定性感知的 epoch-based 图物化框架：抽取阶段仅写入 append-only mention log；每个 epoch 基于固定图快照执行实体解析和 batch 内 mention clustering；解析结果被划分为 merge_existing、create_canonical、create_provisional 和 defer 四类决策；物化阶段通过确定性排序和冲突检测保证写入安全。该框架将实时循环依赖转化为跨 epoch 的受控迭代，在保证 canonical KG 稳定性和可重放性的同时支持新实体的受控增长。
