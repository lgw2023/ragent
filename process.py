...
class CustomerModel:
    ...
    async def _load_async(self) -> None:
        ...
        bundle_paths = resolve_component_bundle_paths(__file__)
        ...
        logger.info(
            "MEP runtime loaded. data_dir=%s source_project_dir=%s runtime_project_dir=%s copied=%s",
    ...
    async def _calc_async(self, req_data):
        ...
        return build_recommend_success(payload)

    def calc(self, req_Data):
        if self._closed:
            return build_recommend_error("CustomerModel has been cleaned up", code="2")
        try:
            return self._loop_runner.run(self._calc_async(req_Data))
        except Exception as exc:
            logger.exception("CustomerModel.calc failed")
            return build_recommend_error(str(exc), code="2")
